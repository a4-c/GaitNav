...
struct ContentView: View {
    ...
    // 创建 CameraManager 实例
    // @StateObject 告诉 SwiftUI：我拥有这个对象，帮我管理它的生命周期
    // 它一被创建就会启动 ARSession 和 LiDAR
    @StateObject private var camera = CameraManager()

    var body: some View {
        ZStack {
            // 底层：ARKit 的摄像头画面
            // 注意这里传的是 camera.session（ARSession 类型）
            // 不再是之前的 AVCaptureSession
            CameraPreview(session: camera.session)
                .ignoresSafeArea()
            ...
            VStack {
                // 把内容推到底部
                Spacer()
                // camera.depthInfo 会实时更新
                // 因为 CameraManager 里每一帧都在修改这个 @Published 属性
                Text(camera.depthInfo)
                    .font(.headline)
                    .foregroundColor(.white)
                    .padding()
                    .background(.black.opacity(0.6))
