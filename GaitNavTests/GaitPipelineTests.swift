...
// GaitPipeline 的属性和嵌套类型（如 StepLengthSource）的 Equatable 一致性
// 被推断为 main-actor-isolated，测试类需要在同一隔离域内执行
//@MainActor
//final class GaitPipelineTests: XCTestCase {
//    
...
//        XCTAssertEqual(gaitPipeline.effectiveStepLength, 0.65, accuracy: 0.001,
//                       "无标定和动态数据时应返回默认步长 0.65m")
//    }
//    
//    // 初始状态的 stepLengthSource 应为 .defaultValue
//    func testStepLengthSource_defaultInitially() {
//        XCTAssertEqual(gaitPipeline.stepLengthSource, .defaultValue)
//    }
//    
//    // stableStepLength 初始状态也应返回默认值
//    func testStableStepLength_defaultValue() {
//        XCTAssertEqual(gaitPipeline.stableStepLength, 0.65, accuracy: 0.001)
//    }
//    
...
//        // 所以不需要创建新的 GaitPipeline（且重复创建会导致 CMMotionManager 冲突崩溃）
//        UserDefaults.standard.set(Float(0.72), forKey: "calibratedStepLength")
//        
//        XCTAssertEqual(gaitPipeline.effectiveStepLength, 0.72, accuracy: 0.001,
//                       "标定后应返回标定步长")
//        XCTAssertEqual(gaitPipeline.stepLengthSource, .calibrated)
//    }
//    
...
//        let steps = gaitPipeline.distanceToSteps(2.0)
...
//        let steps = gaitPipeline.distanceToStableSteps(2.0)
//        let stepsRegular = gaitPipeline.distanceToSteps(2.0)
...
//        let steps = gaitPipeline.distanceToStableSteps(4.0)
...
//        XCTAssertFalse(gaitPipeline.hasEverCalibrated, "从未标定时应返回 false")
//    }
//    
//    // 标定后应返回 true
//    func testHasEverCalibrated_afterCalibration() {
//        UserDefaults.standard.set(Float(0.70), forKey: "calibratedStepLength")
//        XCTAssertTrue(gaitPipeline.hasEverCalibrated, "标定后应返回 true")
//    }
//    
...
//        XCTAssertFalse(gaitPipeline.isDynamicActive, "初始状态动态步长不应活跃")
//    }
//    
...
//        let steps = gaitPipeline.distanceToSteps(distance)
...
//        UserDefaults.standard.set(Float(0.72), forKey: "calibratedStepLength")
//        // 注入动态步长（模拟用户正在走路）
//        gaitPipeline.dynamicEstimator._setForTesting(stepLength: 0.58)
//        
//        XCTAssertEqual(gaitPipeline.effectiveStepLength, 0.58, accuracy: 0.001,
//                       "动态步长应覆盖标定值")
//        XCTAssertEqual(gaitPipeline.stepLengthSource, .dynamic)
//    }
//    
//    // 动态步长活跃时，应覆盖默认值
//    func testEffectiveStepLength_dynamicOverridesDefault() {
//        // 无标定，注入动态步长
//        gaitPipeline.dynamicEstimator._setForTesting(stepLength: 0.70)
//        
//        XCTAssertEqual(gaitPipeline.effectiveStepLength, 0.70, accuracy: 0.001,
//                       "动态步长应覆盖默认值 0.65")
//        XCTAssertEqual(gaitPipeline.stepLengthSource, .dynamic)
//    }
//    
...
//        gaitPipeline.dynamicEstimator.lastUpdateTime = Date().addingTimeInterval(-4.0)
//        
//        XCTAssertEqual(gaitPipeline.effectiveStepLength, 0.72, accuracy: 0.001,
//                       "动态步长超时后应回退到标定值")
//        XCTAssertEqual(gaitPipeline.stepLengthSource, .calibrated)
...
//        gaitPipeline.dynamicEstimator._setForTesting(stepLength: 0.50)
//        
...
//        // 4.0m / 0.80m = 5.0 → ceil = 5
//        let stableSteps = gaitPipeline.distanceToStableSteps(4.0)
//        // distanceToSteps 应使用动态值 0.50
//        // 4.0m / 0.50m = 8.0 → ceil = 8
//        let dynamicSteps = gaitPipeline.distanceToSteps(4.0)
//        
...
//        var stepCount = 0
//        gaitPipeline.onStepDetected = { stepCount += 1 }
//        
...
//        gaitPipeline.processAcceleration(0.95)
//        gaitPipeline.processAcceleration(0.9)
...
//        // stepThreshold = 1.05
//        gaitPipeline.processAcceleration(0.9)   // 基线，设定 lastAcceleration
//        gaitPipeline.processAcceleration(1.1)   // 上升中（> 0.9），isRising = true
//        gaitPipeline.processAcceleration(1.0)   // 下降（< 1.1），波峰 = 1.1 > 1.05 ✓
//        
...
//        // 波峰值 1.02 < 阈值 1.05
//        gaitPipeline.processAcceleration(0.9)
//        gaitPipeline.processAcceleration(1.02)  // 上升
//        gaitPipeline.processAcceleration(0.95)  // 下降 → 波峰 = 1.02 < 1.05
//        
...
//        
//        gaitPipeline.processAcceleration(0.9)
//        gaitPipeline.processAcceleration(1.0)
//        gaitPipeline.processAcceleration(1.1)
//        gaitPipeline.processAcceleration(1.2)
//        
...
//        gaitPipeline.processAcceleration(0.95)
//        gaitPipeline.processAcceleration(0.9)
//        gaitPipeline.processAcceleration(0.85)
//        gaitPipeline.processAcceleration(0.8)
//        
...
//        gaitPipeline.processAcceleration(0.9)
//        gaitPipeline.processAcceleration(1.05)
//        gaitPipeline.processAcceleration(0.95)
//        
...
//        
//        // 先用低于绝对阈值的小波峰建立振动基线
//        gaitPipeline.processAcceleration(1.02)  // > 1.0 初始 → isRising = true
//        gaitPipeline.processAcceleration(1.04)  // 继续上升
//        gaitPipeline.processAcceleration(1.02)  // 下降 → 波峰 1.04 < 1.05 → 不触发，lastValley = 1.02
//        
//        // 这次波峰超过绝对阈值，但振幅很小（车辆振动特征）
//        gaitPipeline.processAcceleration(1.01)  // 下降，lastValley = min(1.02, 1.01) = 1.01
//        gaitPipeline.processAcceleration(1.06)  // 上升，isRising = true
//        gaitPipeline.processAcceleration(1.02)  // 波峰 = 1.06 > 1.05 ✓
...
//        gaitPipeline.processAcceleration(0.97)  // 下降，lastValley = min(1.0, 0.97) = 0.97
//        gaitPipeline.processAcceleration(0.96)  // 继续下降，lastValley = 0.96
//        gaitPipeline.processAcceleration(1.08)  // 上升，isRising = true
//        gaitPipeline.processAcceleration(1.02)  // 波峰 = 1.08 > 1.05 ✓, amplitude = 0.12 > 0.08 ✓
//        
...
//        gaitPipeline.processAcceleration(0.9)   // 下降，lastValley = min(1.0, 0.9) = 0.9
//        gaitPipeline.processAcceleration(0.85)  // 继续下降，lastValley = 0.85
//        gaitPipeline.processAcceleration(1.2)   // 上升，isRising = true
//        gaitPipeline.processAcceleration(1.05)  // 波峰 = 1.2 > 1.05 ✓, amplitude = 0.35 > 0.08 ✓
//        
...
//        for _ in 0..<3 {
//            gaitPipeline.processAcceleration(1.01)
//            gaitPipeline.processAcceleration(1.03)
//            gaitPipeline.processAcceleration(1.06)  // 小幅上升
//            gaitPipeline.processAcceleration(1.03)  // 小幅下降 → 波峰 1.06 > 1.05，但 amplitude 极小
//            gaitPipeline.processAcceleration(1.01)
//            
//            // 等待防抖间隔，确保不是因为防抖而被拒绝
//            RunLoop.current.run(until: Date().addingTimeInterval(0.35))
...
//        gaitPipeline.processAcceleration(0.99)  // 下降，lastValley = min(1.0, 0.99) = 0.99
//        gaitPipeline.processAcceleration(1.06)  // 上升
//        gaitPipeline.processAcceleration(1.0)   // 波峰 = 1.06 > 1.05 ✓, amplitude = 0.07 < 0.08 ✗
//        
...
//        gaitPipeline.processAcceleration(0.98)  // 下降，lastValley = min(1.0, 0.98) = 0.98
//        gaitPipeline.processAcceleration(1.07)  // 上升
//        gaitPipeline.processAcceleration(1.0)   // 波峰 = 1.07 > 1.05 ✓, amplitude = 0.09 > 0.08 ✓
//        
